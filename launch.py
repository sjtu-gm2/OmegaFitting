...
def parse_config(config,entry,dataset,job,scan,scan_id):
    ...
    if 'fix' in parsed_config:
        ...
        for key, value in parsed_config['fix'].items():
...
def run(config):
    ...
    cmd = '../build/MAIN {which_run} {wiggle_file} {wiggle_name:} {lm_file:} {lm_name:} {initial_file:} {initial_name:} {output_dir:} {tag:} {mode:} {max_try:} {start_bin:} {end_bin:} {file_GPRAmp:} {folder_name:}  --fix {fix_pars:} --range {range_pars:}'.format(**config)
