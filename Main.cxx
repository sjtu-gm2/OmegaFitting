...
int main(int argc, char **argv) {
    ...
    if(mode==2) fit_chain = {1029};

    if(mode==3) fit_chain = {1040};
    if(mode==4) fit_chain = {1042};
    if(mode==5) fit_chain = {1044};
    if(mode==6) fit_chain = {1048};

    if(mode==7) fit_chain = {1041};
    if(mode==8) fit_chain = {1043};
    if(mode==9) fit_chain = {1045};
    if(mode==10) fit_chain = {1049};
    ...
    if(mode==40) fit_chain = {1005, 1028, 1029};
    if(mode==41) fit_chain = {2042, 2044};
    ...
    TFile* file_GPRAmp = TFile::Open(argv[14]);
    TString folder = argv[15];
    ...
    FullFit(which_run, wiggle, start_bin, end_bin, lm, outputDir, name, init_values, fit_chain, attempts, fix_parameters, range_parameters, splines);
    return 0;
}